...
This repository contains the code for the ifcSQL_Tools QGIS plugin that allows you to interact with (load, view, filter, query and delete) IFC data stored in the ifcSQL database. 

What can you do with this plugin?
- Load all your georeferenced IFC models into database;
- View all your IFC models simultaneously on a map (in 2D or 3D);
- Filter and display only the IFC elements that interest you;
- Query IFC elements to view their attributes and properties;
...
- Select the IFC file you want to import (For importing into PostgreSQL: the plugin will suggest the last file imported into MSSQL).
- Import the IFC file.
...
### (2) You can delete an IFC file using the “Delete IFC file” button. 
The file will be deleted from both databases or only from MSSQL if it has not alredy been imported into PostgreSQL. The steps to follow are:
